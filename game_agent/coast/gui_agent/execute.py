...
def execute_action(
    ...
    system_prompt=None,
    encoded_image=None,
    gui_model="gpt_operator",
    reasoning_model="gpt-4o",
    type=None,
):
    if gui_model == "gpt_operator":
        return main_gpt_operator(user_prompt=action_prompt)

    elif gui_model == "claude_cua":
        return asyncio.run(
            main_claude_cua(
                user_prompt=action_prompt, system_prompt=system_prompt, type=type
            )
        )
        ...
        return asyncio.run(
            main_claude_sonnet(user_prompt=action_prompt, encoded_image=encoded_image)
        )
